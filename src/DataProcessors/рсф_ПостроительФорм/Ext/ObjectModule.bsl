...
Перем УдаляемыеРеквизиты;
Перем ТекущийЭлемент;
Перем ТекущаяКоманда;
Перем УдаляемыеЭлементы;
Перем Элементы;
Перем Команды;
Перем ПутьРеквизита;
...
Функция ДобавитьРеквизит(Имя, Тип, Заголовок = "", СохраняемыеДанные = Ложь) Экспорт
 
	ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы(Имя, Тип, ПутьРеквизита, Заголовок, СохраняемыеДанные));	
	
	Возврат ЭтотОбъект;

КонецФункции // ДобавитьРеквизит()

Функция УдалитьРеквизит(Имя) Экспорт
 
	УдаляемыеРеквизиты.Добавить(Имя);	
	
	Возврат ЭтотОбъект;

КонецФункции // УдалитьРеквизит()
...
Функция Команда(Имя, Метод) Экспорт
	...
	ТекущаяКоманда.Заголовок = Имя;
	ТекущаяКоманда.Метод = Метод;
	
	Команды.Вставить(Имя, ТекущаяКоманда);
	
	Возврат ЭтотОбъект;

КонецФункции // Команда()
 
Функция ЗаголовокКоманды(Заголовок) Экспорт
	
	СвойствоКоманды("Заголовок", Заголовок);
	
	Возврат ЭтотОбъект;

КонецФункции // ЗаголовокКоманды()
 
Функция ИзменяетСохраняемыеДанные(ИзменяетСохраняемыеДанные) Экспорт
	
	СвойствоКоманды("ИзменяетСохраняемыеДанные", ИзменяетСохраняемыеДанные);
	
	Возврат ЭтотОбъект;

КонецФункции // ИзменяетСохраняемыеДанные()
 
Функция КартинкаКоманды(Картинка) Экспорт
	
	СвойствоКоманды("Картинка", Картинка);
	
	Возврат ЭтотОбъект;

КонецФункции // КартинкаКоманды()
 
Функция ОтображениеКоманды(Отображение) Экспорт
	
	СвойствоКоманды("Отображение", Отображение);
	
	Возврат ЭтотОбъект;

КонецФункции // ОтображениеКоманды()
 
Функция ПодсказкаКоманды(Подсказка) Экспорт
	
	СвойствоКоманды("Подсказка", Подсказка);
	
	Возврат ЭтотОбъект;

КонецФункции // ОтображениеКоманды()

Функция СвойствоКоманды(Имя, Значение) Экспорт
	
	ПроверитьТекущуюКоманду();
	
	ТекущаяКоманда.Вставить(Имя, Значение);
	
	Возврат ЭтотОбъект;

КонецФункции // СвойствоКоманды()
...
Функция УдалитьЭлемент(Имя) Экспорт

    УдаляемыеЭлементы.Добавить(Имя);
	
	Возврат ЭтотОбъект;

КонецФункции // УдалитьЭлемент()
...
Функция Гиперссылка(Гиперссылка) Экспорт 
	...
	Свойство("Гиперссылка", Гиперссылка);
...
Функция МногострочныйРежим(МногострочныйРежим) Экспорт 
	...
	Свойство("МногострочныйРежим", МногострочныйРежим);
...
Функция ПараметрыВыбора(ПараметрыВыбора) Экспорт 
	...
	Свойство("ПараметрыВыбора", ПараметрыВыбора);
...
Функция СквозноеВыравнивание(СквозноеВыравнивание) Экспорт 
	...
	Свойство("СквозноеВыравнивание", СквозноеВыравнивание);
...
Функция ФорматРедактирования(ФорматРедактирования) Экспорт 
	...
	Свойство("ФорматРедактирования", ФорматРедактирования);
...
Функция СоздатьКоманды()
	...
	Для Каждого Команда из Команды Цикл
		...
		Если Форма.Команды.Найти(Команда.Ключ) <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		НоваяКоманда = Форма.Команды.Добавить(Команда.Ключ);
		НоваяКоманда.Заголовок = Команда.Значение.Заголовок;
		НоваяКоманда.Действие = Команда.Значение.Метод;        
		
		СтандартныеСвойства = ПараметрыСозданияКоманды();
		Для Каждого ТекСвойство из Команда.Значение Цикл
...
Процедура УдалитьЭлементы()
	...
	Для Каждого Элемент ИЗ УдаляемыеЭлементы Цикл
		Форма.Элементы.Удалить(Форма.Элементы[Элемент]);	
	КонецЦикла;  
...
Процедура ПроверитьТекущуюКоманду()

	Если ТекущаяКоманда = Неопределено Тогда
		ВызватьИсключение "Команда не инициализирована";
	КонецЕсли;
